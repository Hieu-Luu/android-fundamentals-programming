...
import com.android.databinding.util.cleanSecondsString
import com.android.databinding.util.Timer
 ...
 *
 * It uses different ways to expose data for view:
 * 1.Observable Fields
 * 2.Bindable properties
 * 3.Methods for user actions
 *
 * Data Binding features:
 *
 * <b>One-way data binding</b>
 *
 * See [workTimeLeft] and [restTimeLeft]. They are updated from the ViewModel and the UI is refreshed automatically
 * One-way also applies to user actions. See [WorktiemIncrease] and how a lamda expression in the layout is built:
 *  `android:onClick="@{() -> viewModel.workTimeIncrease()}"`
 *
 * <b>Two-way data binding</b>
 *
 * *Simple: See `timeRunning`. it toggles between start and pause with a single line on the layout:
 *  `android:checked="@{viewModel.timmerRunning}"`
 *
 * *Custom: [numberOfSets] uses the most advanced form of 2-way Data Binding.
 * See [NumberOfSetsBindingAdapters]
 * * */
   ...
   */
    ...
    var numberOfSets: Array<Int> = emptyArray()
        ...
        set(value: Array<Int>) {
            ...
            if (newTotal == numberOfSets[1]) return // Break the loop if there's no change
    ...
    /**
     * Resets timers and state. Called from UI.
     * */
    ...
    private fun timePerSetIncrease(timePerSet: ObservableInt, sign: Int = 1, min: Int = 0) {
        ...
        if (state == TimerStates.STOPPED) {
            // if stopped, update the timers right way
            workTimeLeft.set(timePerWorkSet.get())
            restTimeLeft.set(timePerRestSet.get())
        } else {
